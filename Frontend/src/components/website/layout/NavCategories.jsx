    ...
    const renderAllCategories = (categories) => {
        return categories.map((cat) => (
            <div key={cat.id} className="relative group text-left">
                ...
                {/* Children dropdown */}
                {cat.children && cat.children.length > 0 && (
                    <div className="absolute top-full left-0 hidden group-hover:block bg-white shadow-md rounded z-30 min-w-[180px] p-2 space-y-1">
                        {cat.children.map((child) => (
                            ...
                            >
                                ...
                                {child.children && child.children.length > 0 && activeChildId === child.id && (
                                    <div className="absolute top-0 left-full bg-white shadow-md rounded z-30 min-w-[180px] p-2 space-y-1">
