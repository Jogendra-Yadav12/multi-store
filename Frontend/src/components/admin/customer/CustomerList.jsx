                ...
                <tbody className='text-gray-700 text-left'>
                    {customers.map((cstm, index) => (
                        <tr key={cstm.id} className='border-b border-gray-500/20'>
                            ...
                            <td className='px-2 py-3 text-sm md:text-base'>
                                <span 
                                className={`px-2 py-1 rounded text-sm text-white ${cstm.user_type === 'A' ? 'bg-red-500' : cstm.user_type === 'S' ? 'bg-green-500' : 'bg-blue-500'}`}>
                                {userTypeMap[cstm.user_type]}
                                </span>
                            </td>
                            <td className='px-3'>
                                <div className='flex items-center justify-start gap-2'>
                                    <CreateIcon className='cursor-pointer' onClick={() => editCustomer(cstm.id)} sx={{ fontSize: 25 }} color="primary" fontSize="large" />

                                    <DeleteForeverIcon className='cursor-pointer' onClick={() => deleteCustomer(cstm.id)} sx={{ color: red[500], fontSize: 25 }} fontSize="large" />
                                </div>
                            </td>
                        </tr>
                    ))}
